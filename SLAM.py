...
carStartingPosition = (6,45)#(-38,0)#(6,45)#(3, 3) # coordinates of the starting position of the car
startingCone = [(20,48),(20,42)]#[(-40.5,13), (-35.5,13)] # coordinates of the starting cone
...
while True:
    ...
    if len(seenInnerCones) < 2 or len(seenOuterCones) < 2:
        enoughCones = False
    ...
    if enoughCones:
        ...
        for i in range(0, x_new.__len__()):
            if (x_new[i]**2 + y_new[i]**2) - l_d**2 < 0.3 and (x_new[i]**2 + y_new[i]**2) - l_d**2 > -0.3:
                TP = (x_new[i], y_new[i])
                break  #TODO: implementare il caso in cui non ne trova e abbassare kdd
    else:
        approximatedTrajectory = []
        for point in worldPathPoints[-100:]:
            point = pointFromWorldToCamera(point, inverse_transformation_matrix)
            approximatedTrajectory.append(point)
        #x_new, y_new = zip(*approximatedTrajectory)
        #rightPlot.plot(x_new, y_new, color='black', label='Center Line')
        #plt.pause(40)
        for point in approximatedTrajectory:
            if(point[0]**2 + point[1]**2) - l_d**2 < 0.3 and (point[0]**2 + point[1]**2) - l_d**2 > -0.3:
                TP = point
                break
